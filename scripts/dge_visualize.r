...
plot.volcano <- function(file.dea, output_valcano, control, treat) {
  ...
  dea.table.volcano["group"] <- ifelse(dea.table.volcano$padj>=0.05| dea.table.volcano$padj %in% NA | abs(dea.table.volcano$log2FoldChange) < 0.585,'Not',
                             ifelse(dea.table.volcano$padj < 0.05 & dea.table.volcano$log2FoldChange >= 0.585,'Up','Down'))
