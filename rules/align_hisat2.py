...
rule hisat2_index:
    ...
    shell:
        """
        hisat2-build -p 10 {input.genome} {params.index}
...
if END=="pair":
    rule hisat2:
        ...
        shell:
            """
            hisat2 -p 10 -x {params.index} -1 {input.TrimR1_fastq} -2 {input.TrimR2_fastq} -S {output.sam} --summary-file {params.summary}
            samtools view -@ 10 -bS {output.sam} > {output.bam}
            """
else:
    rule hisat2:
        input:
        ...
        shell:
            """
            hisat2 -p 10 -x {params.index} -U {input.Trim_fastq} -S {output.sam} --summary-file {params.summary}
            samtools view -@ 10 -bS {output.sam} > {output.bam}
            """
...
rule sort:
    ...
    shell:
        """
        samtools sort -@ 10 {input.bam} -o {output.sortbam}
        """