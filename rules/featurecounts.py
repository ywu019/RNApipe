...
if END=='pair':
    rule featureCounts:
        ...
        shell:
            """
            featureCounts -T {params.thread} -p -t exon -g gene_id  -a {input.gtf} -o {output.count_tsv} {input.sortbam} &&\
    ...
    rule featureCounts:
        ...
        shell:
            """
            featureCounts -T {params.thread} -t exon -g gene_id  -a {input.gtf} -o {output.count_tsv} {input.sortbam} &&\
            sed -i '1d' {output.count_tsv} 
            """
