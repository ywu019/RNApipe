...
rule all:
    input:
        volcano = output_path + "/visualize/DEA/limma/volcano_" + control + "_" + treat + ".pdf",
        heatmap = output_path + "/visualize/DEA/limma/heatmap_" + control + "_" + treat + ".pdf"

rule DEA:
    input:
        count_tsv=output_path + path + "/genome_count.tsv"
    output:
        dea = output_path + "/DEA/limma/dea_" + control + "_" + treat + ".tsv",
        deg = output_path + "/DEA/limma/deg_" + control + "_" + treat + ".tsv"
    shell:
        """
        Rscript scripts/dge_limma.r {input.count_tsv} {output.dea} {output.deg}
        sed 's/logFC/log2FoldChange/g' {output.dea} -i
        sed 's/logFC/log2FoldChange/g' {output.deg} -i
        sed 's/adj.P.Val/padj/g' {output.dea} -i
        sed 's/adj.P.Val/padj/g' {output.deg} -i
        """
